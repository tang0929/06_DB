...
- 딕셔너리 뷰 : 데이터 딕셔너리에서 필요한 데이터만 모아서 볼 수 있는 가상 테이블
...
/* 자료형
    ...
    UTF-8 : 영어, 숫자, 기본 특수문자는 1BYTE, 나머지는 3BYTE   
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB)
    CLOB : 대용량 문자 데이터 (4GB)
*/
...
-- MEMBER 테이블 생성
-- 1) ID, PW, 이름, 주민번호, 가입일을 열 이름으로 

-- 2) 컬럼명과 자료형을 같이 생각함
-- MEMBER_ID / VARCHAR2(20)
-- MEMBER_PW / VARCHAR2(20)
-- MEMBER_NAME / VARCHAR2(15) (한글은 1글자당 3BYTE를 차지하므로 최대 5글자)
-- MEMBER_SSN / CHAR(14) (하이픈 포함 14글자 고정임)
-- ENROLL_DATE / DATE
...
-- DEFAULT (=기본값) : 컬럼의 기본 값 지정(필수아님). 값을 넣지 않으면 들어갈 값 지정
);
...
COMMENT ON COLUMN MEMBER.MEMBER_SSN IS '회원주민번호';
...
INSERT INTO MEMBER VALUES ('MEM01','123ABC','홍길동','980101-1234567',TO_DATE('2024-03-07'));
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','234DEF','김영희','980202-1357246',TO_DATE('2024-03-06'));

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03','345GHI','김길동','980303-2468135',DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID,MEMBER_PW,MEMBER_NAME,MEMBER_SSN)
VALUES('MEM04','456JKL','가길동','980404-9876543');
-- DEFAULT를 SYSDATE로 했으므로 굳이 적지 않아도 ENROLL_DATE가 SYSDATE로 반영됨
...
--> NOT NULL 제약 조건에 위배되어 오류 발생 (NULL을 (~) 안에 삽입할 수 없습니다)
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    --USER_ID VARCHAR2(20) UNIQUE,  -- 컬럼 레벨 제약 조건 설정 
    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- USER_ID_U라는 이름의 제약조건 이름을 만들어냈음
    USER_PWD VARCHAR2(30) ,
...
-- ORA-00001: 무결성 제약 조건(KH_NTH.USER_ID_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- 무결성 제약 조건(KH_NTH.USER_NO_PK)에 위배됩니다
...
-- 무결성 제약 조건(KH_NTH.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

-- PRIMARY KEY는 NULL이 들어갈 수 없음
-- 복합키에서 중복은 모든 컬럼을 묶어서 검사하고, NOT NULL은 각 컬럼 별로 검사
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
-- NULL을 ("KH_NTH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
		--> 다른 테이블이 제공하는 컬럼에는 PK 또는 UNIQUE 제약 조건이 설정되어 있어야 함
...
CREATE TABLE USER_USED_FK(
  ...
  -- GRADE_CODE_FK1라는 이름으로 USER_GRADE 테이블의 GRADE_CODE를 참조시킴
);
...
--  무결성 제약조건(KH_NTH.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 참조하는 테이블(USER_GRADE)에 값이 존재하지 않음(50이란 값이 없음)
...
-- GRADE_CODE 30을 삭제 시도
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 30;
SELECT * FROM USER_GRADE;
-- 무결성 제약조건(KH_NTH.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK3(
  ...
  ON DELETE CASCADE -- 부모키 삭제시 참조하는 자식의 행도 모두 삭제
...
CREATE TABLE USER_USED_CHECK(
  ...
  -- 테이블 레벨
  -- CONSTRAINT GENDER_CHECK CHECK(GENDER IN ('남','여')) --> 컬럼 레벨이랑 작성법 동일
);
...
-- 체크 제약조건(KH_NTH.GENDER_CHECK)이 위배되었습니다
...
-- 단, 제약조건은 NOT NULL만 복사되고 다른 제약조건들인 PK, FK, U는 가져오지 못함
-- + DEFAULT, COMMENT 또한 복사 x
