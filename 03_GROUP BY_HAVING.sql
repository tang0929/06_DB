...
SELECT DEPT_CODE 부서코드, SUM(SALARY) "부서 별 급여 합계" -- 그룹 별로 그룹함수(SUM) 연산 수행
FROM EMPLOYEE GROUP BY DEPT_CODE;
-- EMPLOYEE 테이블에서 DEPT_CODE 컬럼 값이 같은 행들끼리 묶음(그룹)
...
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE, SUM(SALARY) "부서 별 급여의 합계", FLOOR(AVG(SALARY)) "부서 별 급여의 평균" ,COUNT(*) "인원 수"
FROM EMPLOYEE 
GROUP BY DEPT_CODE;
...
-- 인원수로 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별, FLOOR(AVG(SALARY)) 평균, SUM(SALARY) 합계, COUNT(*) "인원 수"
FROM EMPLOYEE
...
SELECT JOB_CODE "직급 코드", SUM(SALARY) "2000년도 이후 입사자들 급여 합"
FROM EMPLOYEE
...
ORDER BY DEPT_CODE;
-- 2개 이상의 행을 출력하고자 할 때 하나만 GROUP 처리하면 출력하고자 하는 행이 안맞음
-- 모두 그룹함수 처리를 해줘야 함
...
SELECT DEPT_CODE 부서코드, SUM(SALARY) FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 부서 코드 오름차순으로 정렬하세요.
SELECT MAX(SALARY),MIN(SALARY), DEPT_CODE 
FROM EMPLOYEE
GROUP BY DEPT_CODE 
ORDER BY DEPT_CODE;
...
-- 직급코드 오름차순으로 정렬하세요
SELECT COUNT(BONUS), JOB_CODE FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
-- 부서 코드 오름차순으로 정렬하세요
SELECT DEPT_CODE "부서 코드", FLOOR(AVG(SALARY)) "급여 평균"
FROM EMPLOYEE 
...
-- 부서 코드가 'D5' 또는 'D6'인 사원의 이름, 부서코드 조회
...
-- 부서코드가 'D5'이면서 급여가 300만 초과하는 사원 이름, 부서코드, 급여 조회
...
-- 부서코드가 'D5'이거나 급여가 300만 초과하는 사원의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE DEPT_CODE = 'D5'
UNION ALL
SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE SALARY > 3000000;
...
-- 집합 연산자 실제 사용 예시
-- 집합 연산자 사용시 컬럼의 타입만 일치하면 연산 수행이 가능하다.
