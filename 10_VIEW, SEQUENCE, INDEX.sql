 ...
 *  [WITH READ OLNY];
 * 
 *  CREATE VIEW 뷰이름 AS 서브쿼리(SELECT 문);
 * 
 *  1) OR REPLACE 옵션 : 
...
GRANT CREATE VIEW TO KH_NTH;
-- 관리자 계정으로 KH_NTH 계정에게 VIEW를 CREATE할 수 있는 권한을 부여함
...
-- OR REPLACE 옵션 사용하기
--> VIEW 생성 시 같은 이름의 VIEW가 이미 있으면 변경
...
-- OR REPLACE를 안쓰면 위에서 이미 V_EMP VIEW를 생성했으므로 기존의 객체가 이름을 이미 사용중이라는 오류 발생
-- OR REPLACE를 붙여야 정상적으로 V_EMP라는 이름의 VIEW가 만들어짐
SELECT * FROM V_EMP;
...
-- DEPT_TITLE 컬럼만 NULL 허용된 상태


-- DEPT_COPY2 테이블의 DEPT_ID, LOCATION_ID 컬럼만을 이용해 V_COPY2 VIEW 생성
CREATE VIEW V_COPY2 AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2;

SELECT * FROM V_COPY2;


-- V_COPY2 VIEW에 INSERT하기
INSERT INTO V_COPY2 VALUES('D0','L2');


-- 원본 테이블 INSERT 확인
SELECT * FROM DEPT_COPY2; -- 기존 테이블에도 값이 삽입되었음을 확인.
--> 가상 테이블 VIEW는 값 저장/수정/삭제가 불가능하여 연결된 원본이 변함


-- D0 / NULL / L2 
-- NULL은 DB 무결성을 약하게 만드는 원인이기 때문에 가능한 NULL이 안생기게 하는 것이 좋음



-- WITH READ ONLY 옵션을 추가해 읽기 전용으로 변경
CREATE OR REPLACE VIEW V_COPY2 AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2
WITH READ ONLY; 
-- VIEW를 INSERT 하려고 하면 오류발생
--  ORA-42399: 읽기 전용 뷰에서는 DML 작업을 수행할 수 없습니다.



ROLLBACK;
-- VIEW와 DEPT_COPY2 모두 값이 사라짐 
----------------------------------------------------------------
...
MAXVALUE 150 -- 증가한 값 상한치
NOMINVALUE -- 최소값은 없음
NOCYCLE -- 반복없음
NOCACHE -- 미리 생성해두는 SEQUENCE 번호 없음
;
...
-- MAXVALUE를 넘게되면 다음과 같은 오류 발생



-- TB_TEST 테이블 INSERT 시 SEQUENCE 사용
...
-- INDEX 사용 방법 : WHERE 절에 INDEX가 지정된 컬럼을 언급하기

SELECT * FROM EMPLOYEE; -- INDEX 미사용

SELECT * FROM EMPLOYEE WHERE EMP_ID != 0; -- 조건을 뭘로하든 자동 생성된 INDEX를 언급만 시킴
...
-- INDEX 사용 안하고 50만번째 데이터 찾기
SELECT * FROM TB_IDX_TEST WHERE TEST_ID = 'TEST500000'; -- 0.011s ~ 0.016s 소요


-- INDEX 사용
SELECT * FROM TB_IDX_TEST WHERE TEST_NO = 500000; -- 0.000s ~ 0.002s 소요
