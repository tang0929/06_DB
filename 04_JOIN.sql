...
/* JOIN은 단순히 테이블을 2개 붙이는 것이 아닌 
 * 기준 삼은 테이블의 한 컬럼을 지정해 
 * 다른 테이블의 한 컬럼과 같은 행을 찾아 기준 테이블 옆에 한 행씩 붙여나간다.*/
...
SELECT DEPT_TITLE 부서명, LOCAL_NAME 지역명 FROM DEPARTMENT
...
-- 부서명이 '해외영업2부'인 사원의 사번, 이름, 부서명을 사번 오름차순으로 조회
SELECT EMP_ID 사번,EMP_NAME 이름,DEPT_TITLE 부서명 
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE DEPT_TITLE = '해외영업2부'
ORDER BY 사번;
-- 해석 순서 : (FROM - JOIN) - WHERE - SELECT - ORDER BY
...
-- EMPLOYEE와 JOB 테이블은 둘다 JOB_CODE를 공통적으로 사용한다
JOIN JOB USING(JOB_CODE);
-- JOIN ON과는 달리 한 테이블로 합칠 때  컬럼이 중복으로 나오게 되므로 한 번만 출력하도록 함
...
-- 누구의 JOB_CODE인지 적어두지 않으면 '열의 정의가 애매합니다'라는 오류 발생
-- 어떤 테이블의 JOB_CODE인지 알려주기 위해 EMPLOYEE.JOB_CODE라고 작성함
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--> DEPT_CODE가 NULL 값을 가진 하동운, 이오리가 누락됨. 연결되지 않아 최종 결과에서 제외됨. == INNER JOIN의 특성
...
-->  *반드시 OUTER JOIN임을 명시해야 한다. OUTER를 생략하면 INNER JOIN 취급을 하기 때문
...
-- JOIN 구문이 있는 위치 기준으로 왼쪽에 작성된 테이블(EMPLOYEE)의 모든 행이 최종 결과(RESULT SET)에 포함되도록 한다.
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
--> DEPT_CODE의 값이 DEPT_ID와 일치하지 않아도 결과에 포함시키라는 뜻


-- 2) RIGHT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 오른편에 기술된 테이블의 컬럼 수를 기준으로 JOIN
-- JOIN 구문이 있는 위치 기준으로 오른쪽 에 작성된 테이블(DEPARTMENT)의 모든 행이 최종 결과(RESULT SET)에 포함되도록 한다.
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
...
-- JOIN 구문이 있는 위치 기준으로 양쪽에 작성된 테이블의 모든 행이 최종 결과(RESULT SET)에 포함되도록 한다.
...
-- 모든 경우의 수를 보고 싶을 때 빼고는 직접 작성하는 경우는 없음
-- 자연 조인(NATURAL JOIN) 실패 결과로 교차 조인 결과가 출력됨 -> 교차 조인의 결과가 보이면 잘못쓴 것

SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 23 * 9 = 207행의 결과값 출력
...
-- 사원의 급여가 SAL_LEVEL에 작성된 최소~최대(MIN_SAL ~ MAX_SAL) 범위의 급여가 맞을 때에만 결과에 포함시키는 JOIN

SELECT EMP_NAME, SALARY, MIN_SAL ,SALARY, MAX_SAL 
FROM EMPLOYEE e 
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음 --> 똑같은 테이블이 2개가 있다

-- 각 사원, 사수 이름 조회

-- ANSI 표준
SELECT EMP.EMP_NAME 사원명, MGR.EMP_NAME 사수명 FROM EMPLOYEE EMP
LEFT JOIN EMPLOYEE MGR ON(EMP.MANAGER_ID = MGR.EMP_ID);


-- 오라클 구문
SELECT EMP.EMP_NAME 사원명, MGR.EMP_NAME 사수명 FROM EMPLOYEE EMP,EMPLOYEE MGR
...
-- JOB.JOB_CODE     (CHAR(2))
--> 컬럼명과 자료형이 모두 일치하여 NATURAL JOIN 대상
...
-- NATURAL JOIN 실패시, CROSS JOIN의 결과가 조회된다.
...
-- 왼쪽 2개를 먼저 JOIN하고 그 JOIN된 것을 LOCATION과 JOIN 시킴
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) 
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE); 
-- 1) EMPLOYEE와 DEPARTMENT를 조인
-- 2) 1차 JOIN 결과에 LOCATION 2차 조인
...
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- EMPLOYEE에 LOCATION_ID가 없어 부적합한 식별자라고 오류 발생
...
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_CODE 직급명, DEPT_TITLE 부서명, LOCAL_CODE 근무지역, SALARY 급여
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME 사원명, JOB_NAME 직급명, SALARY 급여, SAL_LEVEL, BONUS ,SALARY*12*(1+NVL(BONUS,0)) "연봉(보너스포함)"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
