...
-- 제약 조건은 수정 개념이 없기 때문에 삭제하게 되면 무조건 다시 생성을 해야한다. 
...
--> 일부 값이 10BYTE보다 큰 경우가 있어서 10으로 수정할 수 없음
...
-- LNAME 기본값을 변경한 내용을 가지고 UPDATE 하기
...
-- 마지막 남은 DEPT_ID 삭제 시도
ALTER TABLE DEPT_COPY 
DROP(DEPT_ID); 
-- 테이블의 모든 열들을 삭제할 수 없다면서 삭제 불가. 최소 1개의 열이 있어야 함
SELECT * FROM DEPT_COPY;
